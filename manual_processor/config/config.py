...
Backward-compatible configuration wrapper that uses Pydantic Settings internally
while preserving the legacy API (Config.get_instance(), AppConfig, etc.).
"""
...
class AppConfig:
    ...
    Internally uses Pydantic Settings while preserving
    ...
    def output_directory(self, value: Path) -> None:
        self._settings.paths.output_directory = str(value)
